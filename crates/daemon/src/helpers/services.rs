...
use crate::daemon::GateDaemon;
use gate_core::access::PermissionManager;
...
impl<'a> ServiceAccessor<'a> {
    ...
    pub fn new(daemon: &'a GateDaemon) -> Self {
        Self { daemon }
    }
    
    /// Get the permission manager with standard error handling
    pub async fn permission_manager(&self) -> Result<Arc<dyn PermissionManager>, HttpError> {
        self.daemon
            .get_permission_manager()
            .await
            .map_err(|e| HttpError::InternalServerError(format!("Failed to get permission manager: {}", e)))
    }
    ...
    pub async fn state_backend(&self) -> Result<Arc<dyn StateBackend>, HttpError> {
        self.daemon
            .get_state_backend()
            .await
            .map_err(|e| HttpError::InternalServerError(format!("Failed to get state backend: {}", e)))
    }
    
    /// Get both permission manager and state backend (common pattern)
    pub async fn core_services(&self) -> Result<(Arc<dyn PermissionManager>, Arc<dyn StateBackend>), HttpError> {
